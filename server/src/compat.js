...
export function handleCompatRequest (path, body, userId, repo, runtime) {
  ...
  if (path === '/game/player/level-up') {
    ...
    // Full cost/reward computation is GameData-dependent. Returning a stable
    // level object keeps the endpoint explicit until that calculation is
    // implemented, instead of falsely treating an unknown request as success.
