...
- `slayersclub.bethesda.net` (24 bytes, host **ancilar**) aparece **duas vezes** dentro de `assets/bin/Data/Managed/Metadata/global-metadata.dat`, **não** em `assets/aa/` — a suposição anterior (bundle Addressable) estava errada **para este host**. O host da **API de gameplay** (`international.gear.bethesda.net`, 31 bytes) é outro caso: vive em bundle Addressables (ver bullet do pipeline completo abaixo). As duas ocorrências são a URL completa `https://slayersclub.bethesda.net/` (33 bytes). `scripts/analyze_apk.py` e `scripts/patch_apk.py` agora escaneiam `global-metadata.dat` também.
