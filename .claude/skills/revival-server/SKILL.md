...
3. Responda com `ok(res, data)` / `fail(res, status, code)` — nunca monte o JSON na
   mão, o envelope é do `protocol.js`.
...
- `active: false` desliga; `always: true` ignora janela de tempo; caso contrário
  `start_time`/`end_time` (ISO ou epoch) definem a janela.
...
`db.js: createUser()` emite `randomBytes(32).toString('base64url')` — string
opaca. O cliente loga:
...
- `research_mode` (env `RESEARCH_MODE`, default `true`) e `archive_mode` mudam o
  comportamento de battle pass/eventos.
