...
The module deliberately touches only serializable TextAsset/MonoBehaviour values
that contain one of the known official hosts. It never performs variable-length
raw binary replacement. UnityPy is loaded lazily so the legacy exact-length
patch path keeps working even when the optional dependency is absent.
"""
    ...
    for obj in env.objects:
        ...
        if type_name == "TextAsset":
            try:
                ...
                # A malformed/unusual TextAsset must not abort scanning other
                # serializable objects in the same bundle.
                continue
            ...
            try:
                ...
                # IL2CPP MonoBehaviours without a usable typetree are expected;
                # leave them untouched instead of guessing binary offsets.
                continue
...
def patch_bundle(path: Path, target_host: str) -> dict[str, Any]:
    ...
    try:
        ...
        if verification["official"] > 0:
            raise RuntimeError("bundle reserializado ainda contém hostname oficial em objetos modificáveis")
        temp.replace(path)
    finally:
        if temp.exists():
            temp.unlink()
...
def main() -> int:
    ...
    try:
        ...
        print(f"Falha no patch bundle-aware: {exc}")
        return 4
